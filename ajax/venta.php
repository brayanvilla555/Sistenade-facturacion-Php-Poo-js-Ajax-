...
switch ($_GET["op"]){
	...
	case 'listar':
		$rspta=$venta->listar();
 		...
 		while ($reg = $rspta->fetch_object()){
 			//condicianamos  para ver si es Ticket o Boleta
 			if ($reg->tipo_comprobante == 'Ticket') {
 				//declaramos la tura del archivo lo lo concatenamos con el ID
 				$url = '../reportes/exportarTicketVenta.php?id=';
 			}else{
 				//declaramos la tura del archivo lo lo concatenamos con el ID
 				$url = '../reportes/exportarFacturaVenta.php?id=';
 			}
 			$data[]=array(
 				"0"=>(($reg->estado=='Aceptado')?'<button class="btn btn-warning btn-sm" onclick="mostrar('.$reg->idventa.')"><i class="fa fa-eye"></i></button>'.
 				    ...
 				    ' <a target="_blank" href="'.$url.$reg->idventa.'"><button class="btn btn-info btn-sm"><i class="fa fa-file"></i></button></a>',
 				"1"=>$reg->fecha,
 				"2"=>$reg->cliente,
 				"3"=>$reg->usuario,
